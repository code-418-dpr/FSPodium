...
export function RequestDialog({ open, onOpenChange, action, selectedRequestId, forEvents }: Props) {
    ...
    function handleConfirm(): void {
        ...
        if (forEvents && action === "decline") {
            declineEventRequest(selectedRequestId, refusalReason);
        } else if (action === "accept") {
            adminChangeRepresentationRequestStatus(selectedRequestId, true);
        } else {
            adminChangeRepresentationRequestStatus(selectedRequestId, false, refusalReason);
        }
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>
                        {action === "accept" ? "Принять заявку" : "Отклонить заявку"}
                    </DialogTitle>
                    <DialogDescription>
                        {action === "accept"
