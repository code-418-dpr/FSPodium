...
KEYWORDS = (
    {'лет', 'младше', 'старше'}
    | _event_participants.MALE_KEYWORDS
    | _event_participants.FEMALE_KEYWORDS
)
...
def parse(text: str, no_empty_sport: bool = True) -> list[dict]:
    global sport, team
    data = []
    state = State.SPORT_TEAM_ID
    id_ = title = start = end = \
        participants_count = country = city = participants = categories = ''

    def reset_variables() -> None:
        nonlocal id_, title, start, end, participants_count, \
                 country, city, participants, categories
        id_ = title = start = end = \
            participants_count = country = city = participants = categories = ''
        ...
        match state:
            ...
            case State.CITY_PARTICIPANTS_COUNT:
                if not line.isdigit():
                    ...
                    if not no_empty_sport or sport:
                        data.append({
                            'sport': sport,
                            'team': team,
                            'id': id_,
