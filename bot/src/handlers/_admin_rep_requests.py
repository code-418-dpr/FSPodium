...
ADMIN_REP_REGEX = re.compile(r'^rep([\+\-])(.+)$')

logger = logging.getLogger(__name__)


@bot.on_callback_query(
        filters.regex(ADMIN_REP_REGEX) 
        & my_filters.admin
)
async def handle_keyboard(_, query: types.CallbackQuery) -> None:
    approve_or_refuse, req_id = ADMIN_REP_REGEX.match(str(query.data)).groups()
    if approve_or_refuse == '+':
        res = await http_client.post(
            f'{WEB_URL}/api/request', 
            json={'requestId': req_id, 'type': 'representation', 'refusalReason': None}
        )
        ...
        fsm[query] = State('wait_for_reason', f'{req_id}|{query.message.id}')


@bot.on_message(
        filters.text
        & my_filters.admin
        & fsm.at('wait_for_reason')
)
